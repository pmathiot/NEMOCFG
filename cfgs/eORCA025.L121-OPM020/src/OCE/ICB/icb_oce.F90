MODULE icb_oce
   ...
   TYPE, PUBLIC ::   point              !: properties of an individual iceberg (position, mass, size, etc...)
      ...
      REAL(wp) ::   ssu, ssv, ui, vi, ua, va, ssh_x, ssh_y, sst, cn, hi, sss  ! properties of iceberg environment 
   ...
   REAL(wp), PUBLIC, DIMENSION(:,:), ALLOCATABLE ::   sst_e, fr_e, sss_e
...
!TOM> no horizontal drag for sea ice! real, PARAMETER :: pp_Cd_ih=0.0012 ! (lateral) Drag coeff. between bergs and sea-ice
   ...
   CHARACTER(lc), PUBLIC ::   cn_icbrst_in     !: suffix of ocean restart name (input)
   CHARACTER(lc), PUBLIC ::   cn_icbrst_indir  !: restart input directory
   CHARACTER(lc), PUBLIC ::   cn_icbrst_out    !: suffix of ocean restart name (output)
   CHARACTER(lc), PUBLIC ::   cn_icbrst_outdir !: restart output directory
   CHARACTER(lc), PUBLIC ::   cn_icbdir_trj
#endif
   !
