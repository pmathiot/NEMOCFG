...
902   IF( ios >  0 ) CALL ctl_nam ( ios , 'namzdf_tke in configuration namelist' )
      ...
      IF( nn_etau /= 0 ) THEN      
         ...
         CASE( 2 )                                 ! F(latitude) : 0.5m to 30m poleward of 40 degrees (NH)
            WHERE ( gphit(:,:) > 0.0_wp )          !               0.5m to 40m poleward of 40 degrees (SH)
               htau(:,:) = MAX(  0.5_wp,        MIN( 30._wp, 45._wp* ABS( SIN( rpi/180._wp * gphit(:,:) ) ) ) )
            ELSEWHERE
               htau(:,:) = MAX(  0.5_wp, 1.33 * MIN( 30._wp, 45._wp* ABS( SIN( rpi/180._wp * gphit(:,:) ) ) ) )
            END WHERE
         END SELECT
      ENDIF
