      ...
      DO jn = 1, kjpt                                 !  tracer loop  !
         ...
         IF(  ( cdtype == 'TRA' .AND. ( jn == jp_tem .OR. ( jn == jp_sal .AND. ln_zdfddm ) ) ) .OR.   &
            ...
            IF( ln_zad_Aimp ) THEN         ! Adaptive implicit vertical advection
               DO jk = 1, jpkm1
                  DO jj = 2, jpjm1
                     DO ji = 2, jpim1   ! vector opt. (ensure same order of calculation as below if wi=0.)
            ...
            DO jj = 2, jpjm1        !* 1st recurrence:   Tk = Dk - Ik Sk-1 / Tk-1   (increasing k)
               DO ji = 2, jpim1            ! done one for all passive tracers (so included in the IF instruction)
